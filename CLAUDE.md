...
6. **No Firebase Auth:** The app still runs on alliance trust, not real user identity — do not add Firebase Auth, custom claims, or Cloud Functions unless explicitly requested (this project intentionally stays on the free Spark plan). Login/password handling for the shared `accounts` collection DOES exist, hardened as far as possible without a backend: passwords are never stored or read in plaintext (PBKDF2-hashed client-side, see `core/utils/password.util.ts`), the `accounts` collection is fully unreadable, and "login" is a Firestore-rules-verified write, not a client-side read+compare (see `core/services/auth.service.ts` and, in the `plannet-wos` repo, `firestore.rules` — **that repo is now the sole owner/deployer of Firestore rules for the shared `tal-coordinator` project; this repo doesn't carry a copy anymore**, see its README). New admin accounts are provisioned manually now, not self-service from the superadmin dashboard. This does NOT extend to wiki_articles/players/tasks/etc. — those collections remain open, alliance-trust, unauthenticated by design.
...
- Do not add Firebase Auth, custom claims, or Cloud Functions (see rule 6 — the existing password-hash login is the intentional ceiling, not a stepping stone toward real Auth).
